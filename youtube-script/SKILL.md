...
```bash
~/QCLI/youtube-script/venv/bin/python scripts/extract_transcript.py <URL>
```
...
```bash
python scripts/extract_transcript.py <URL> [OPTIONS]
```
...
- **No captions available**: Some videos have captions disabled. Inform the user and suggest alternatives.
